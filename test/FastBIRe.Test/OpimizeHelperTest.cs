namespace FastBIRe.Test
{
    ...
    public class OpimizeHelperTest
    {
        ...
        public void MySql()
        {
            var act=OpimizeHelper.Sql("table", SqlType.MySql);
            Assert.AreEqual("OPTIMIZE TABLE `table`;", act);
        }
        [TestMethod]
        public void SqlServer()
        {
            var act = OpimizeHelper.Sql("table", SqlType.SqlServer);
            Assert.AreEqual("ALTER INDEX ALL ON [table] REBUILD;", act);
        }
        [TestMethod]
        public void Sqlite()
        {
            var act = OpimizeHelper.Sql("table", SqlType.SQLite);
            Assert.AreEqual("VACUUM;", act);
        }
        [TestMethod]
        public void PostgreSql()
        {
            var act = OpimizeHelper.Sql("table", SqlType.PostgreSql);
            Assert.AreEqual("VACUUM FULL \"table\";", act);
        }
        [TestMethod]
        public void Oracle()
        {
            var act = OpimizeHelper.Sql("table", SqlType.Oracle);
            Assert.AreEqual("ALTER TABLE TRUNCATE TABLE \"table\" MOVE;", act);
        }
        [TestMethod]
        public void DB2()
        {
            var act = OpimizeHelper.Sql("table", SqlType.Db2);

...
namespace FastBIRe
{
    public partial class DefaultScriptExecuter
    {
        ...
        {
            using (var activity = ScriptExecuterActivity.ScriptExecuterActivitySource.StartActivity("ExecuteAsync"))
            {
                script = SqlQutoConversion(SqlParameterConversion(script));
                    ...
                    using (var batch = Connection.CreateBatch())
                    {
                        try
                        {
                            ...
                            foreach (var item in s.ScriptUnits!)
                            {
                                ...
                                comm.CommandText = SqlQutoConversion(SqlParameterConversion(item.Script));
        ...
        {
            using (var activity = ScriptExecuterActivity.ScriptExecuterActivitySource.StartActivity("ReadResultAsync"))
            {
                script = SqlQutoConversion(SqlParameterConversion(script));
        ...
        {
            using (var activity = ScriptExecuterActivity.ScriptExecuterActivitySource.StartActivity("ReadAsync"))
            {
                script = SqlQutoConversion(SqlParameterConversion(script));

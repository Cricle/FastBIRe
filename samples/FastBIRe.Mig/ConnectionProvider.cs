        ...
        public static DbConnection GetDbMigration(SqlType type, string database, bool mariadb = false)
        {
            ...
            switch (type)
            {
                ...
                case SqlType.MySql:
                    ...
                    {
                        conn = new MySqlConnection($"Server=192.168.1.101;Port=3306;Uid=root;Pwd=Syc123456.;Connection Timeout=2000;Character Set=utf8{(string.IsNullOrEmpty(database) ? string.Empty : $";Database={database};")}");
                    }
                    break;
                case SqlType.SQLite:

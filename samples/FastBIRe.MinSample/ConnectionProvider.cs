        ...
        public static MigrationService GetDbMigration(SqlType type, string database, bool mariadb = false)
        {
            ...
            switch (type)
            {
                ...
                case SqlType.SqlServer:
                    conn = new SqlConnection($"Server=192.168.2.45;Uid=sa;Pwd=123456;Connection Timeout=2000;TrustServerCertificate=true{(string.IsNullOrEmpty(database) ? string.Empty : $";Database={database};")}");
                    break;
                case SqlType.MySql:
                    if (mariadb)
                    {
                    ...
                    {
                        conn = new MySqlConnection($"Server=192.168.1.100;Port=3306;Uid=root;Pwd=syc123;Connection Timeout=2000;Character Set=utf8{(string.IsNullOrEmpty(database) ? string.Empty : $";Database={database};")}");
                    }
                    break;
                case SqlType.SQLite:
